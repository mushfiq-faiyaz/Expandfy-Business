...
function sumExpensesForYear(expenses: Expense[], y: number): number {
  return expenses.reduce((sum, e) => {
...
function sumIncomeForYear(entries: IncomeEntry[], y: number): number {
  return entries.reduce((sum, e) => {
    const incDate =
      (e as unknown as { targetDate?: string; date?: string }).targetDate ||
      (e as unknown as { targetDate?: string; date?: string }).date
    const d = incDate
      ? new Date(incDate.includes('T') ? incDate : incDate + 'T12:00:00')
